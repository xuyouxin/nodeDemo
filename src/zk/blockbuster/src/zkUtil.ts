...
import { Client, CreateMode } from "node-zookeeper-client";
...
export class ZKUtil {
  ...
  // 创建节点前，得先确保父节点存在
  static async createNode(path: string, data: any = ""): Promise<boolean> {
    return new Promise(resolve => {
      this.client.create(path, Buffer.from(JSON.stringify(data)), CreateMode.PERSISTENT, function (error, path) {
        if (error) {
