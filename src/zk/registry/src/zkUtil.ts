...
import { Client, CreateMode, Event, Exception } from 'node-zookeeper-client';
...
export class ZKUtil {
  ...
  static async getChildren(path: string, watcher: (event: Event) => void = undefined): Promise<string[]> {
    return new Promise(resolve => {
  ...
  static async getData(path: string, watcher: (event: Event) => void = undefined): Promise<string> {
    return new Promise(resolve => {
  ...
  static async setData(path: string, data: any): Promise<boolean> {
    return new Promise(resolve => {
  ...
  static async clearAllData(rootPath: string = "/") {
    const children = await this.getChildren(rootPath);
    for (const name of children) {
      if (name != "zookeeper") {
        await this.removeRecursive(`${rootPath}/${name}`);
      }
    }
  }
}
